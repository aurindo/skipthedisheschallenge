...
public class LiveTest {
    ...
    private final FormAuthConfig formConfig = new FormAuthConfig("/login", "user", "password");
    ...
    @Test
    public void whenDoLogin_thenSuccess() {
        final Response response = RestAssured.get(ROOT_URI);
    ...
    public void whenGetAllBooks_thenSuccess() {
        final Response response = RestAssured.get(ROOT_URI + "/order-service/");
        Assert.assertEquals(HttpStatus.FOUND.value(), response.getStatusCode());
        Assert.assertNotNull(response.getBody());
    }
